...
Status: in progress. AgentScope `2.0.0` dependency and construction APIs are verified; runtime wiring, session persistence boundary, Redis storage configuration, and focused SSE routes are in place. Legacy API replacement and UAT refresh are still pending.
