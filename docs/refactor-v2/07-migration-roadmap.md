...
Status: code validation complete; external UAT and tracing verification pending. AgentScope `2.0.0` dependency and construction APIs are verified; runtime wiring, session persistence boundary, Redis storage configuration, focused SSE routes, v2 app entrypoint wiring, local deterministic function tools, and v2 UAT request helpers are in place. Legacy v1 test cleanup, real-service UAT, and Studio/Phoenix trace verification move with the API/UAT refresh work.
...
- Replaced the main app entrypoint with a FastAPI v2 app that registers only `/v2/runtimes/init` and `/v2/chat`.
- Added project-local deterministic function tool support through AgentScope v2 `FunctionTool` for `get_weather`, `calculate`, `run_platform_report`, and `summarize_platform_callable`.
- Updated UAT helpers to translate legacy bootstrap payloads into v2 runtime capabilities and v2 chat requests.
...
- Use AgentScope v2 RedisStorage for framework session and memory state when `SESSION_BACKEND=redis`.
...
- `uv run pytest tests/test_refactor_v2_schemas.py tests/test_refactor_v2_runtime_manager.py tests/test_agentscope_v2_adapter.py tests/test_agentscope_v2_runtime_builder.py tests/test_runtime_factory.py tests/test_agentscope_v2_execution.py tests/test_agentscope_v2_event_mapping.py tests/test_v2_api.py tests/test_agentscope_v2_session_store.py tests/test_agentscope_v2_storage.py tests/test_v2_uat_helpers.py tests/test_tools.py tests/test_skill_runtime.py tests/test_skill_local_runtime.py -q` passes.
- `uv run ruff check src --fix` and targeted ruff checks pass.
- Full test collection currently fails on old v1 imports such as `agentscope.memory`, `agentscope.tracing`, `StdIOStatefulClient`, `StatefulClientBase`, and `ReActAgent`. Legacy tool helper import failures have been removed. Remaining failures belong to the legacy API/UAT cleanup.
- Tool, MCP, skill, remote skill, and session UATs still need to be run against a real AgentScope v2 service.
- Effective model and prompt hash appear in streamed events.
- Trace metadata is included in platform events when present on the runtime profile; Studio or Phoenix end-to-end verification is still pending.
